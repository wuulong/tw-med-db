...
from src.cli.commands_h41 import h41_app, resolve_db_path, get_sqlite_connection
...
class TestM16TWEHRDB(unittest.TestCase):
    ...
    def test_02_search_official_and_synthea(self):
        ...
        print("="*80)
        res = runner.invoke(h41_app, ["search", "pat-example", "--db", self.db_path])
        print(f"  ➜ CLI 搜尋官方病患結果:\n{res.stdout.strip()}")
        self.assertEqual(res.exit_code, 0)
        self.assertIn("陳加玲", res.stdout)

        res_syn = runner.invoke(h41_app, ["search", "pat-synthea-", "--db", self.db_path])
    ...
    def test_03_vitals_command_time_series(self):
        ...
        print("="*80)
        res = runner.invoke(h41_app, ["vitals", "pat-example", "--db", self.db_path])
        self.assertEqual(res.exit_code, 0)
    ...
    def test_04_fhir_export_synthea_compatibility(self):
        ...
        print("="*80)
        res = runner.invoke(h41_app, ["fhir-export", "pat-example", "--db", self.db_path])
        self.assertEqual(res.exit_code, 0)
    ...
    def test_05_cross_journey_command(self):
        ...
        print("="*80)
        res = runner.invoke(h41_app, ["cross-journey", "pat-example", "--db", self.db_path])
        self.assertEqual(res.exit_code, 0)
    ...
    def test_06_status_data_origin_breakdown(self):
        ...
        print("="*80)
        res = runner.invoke(h41_app, ["status", "--db", self.db_path, "-j"])
        self.assertEqual(res.exit_code, 0)
