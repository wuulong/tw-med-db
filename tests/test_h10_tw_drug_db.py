...
from modules.h10_tw_drug_db.fts import search_m01_fts
...
class TestM01TwDrugDbDomain(unittest.TestCase):
    ...
    def test_m01_02_primary_key_integrity_and_zfill(self):
        ...
        # 採樣檢驗 drug_code 長度大多為 14 碼 (DHA/DHY 開頭之 14 位通關簽審藥碼，少部分早期歷史許可為 13 碼內衛藥製字)
        cursor.execute("SELECT drug_code FROM m01_tw_drug_db LIMIT 1000;")
        codes = [r[0] for r in cursor.fetchall()]
        valid_lengths = [len(c) in (13, 14) for c in codes]
        print(f"  ➜ 採樣 1,000 筆通關簽審藥碼長度合規率: {sum(valid_lengths)}/1000")
        self.assertGreaterEqual(sum(valid_lengths), 990)
        conn.close()
    ...
    def test_m01_07_cli_runner_e2e_commands(self):
        ...
        # 測試 m01 search 指令
        res_search = runner.invoke(app, ["h10", "search", "普拿疼", "--db", self.db_path])
